...
if (isset($_POST['submit'])) {
    $application_date = mysqli_real_escape_string($db, $_POST['application_date'] ?? '');
    $name = mysqli_real_escape_string($db, $_POST['name'] ?? '');
    $staff_code = mysqli_real_escape_string($db, $_POST['staff_code'] ?? '');
    $father = mysqli_real_escape_string($db, $_POST['father'] ?? '');
    $mother = mysqli_real_escape_string($db, $_POST['mother'] ?? '');
    $gender = mysqli_real_escape_string($db, $_POST['gender'] ?? '');
    $guardian_occupation = mysqli_real_escape_string($db, $_POST['guardian_occupation'] ?? '');
    $religion = mysqli_real_escape_string($db, $_POST['religion'] ?? '');
    $religion_category = mysqli_real_escape_string($db, $_POST['religion_category'] ?? '');
    ...
    $identity_type = mysqli_real_escape_string($db, $_POST['identity_type'] ?? '');
    $identity_no = mysqli_real_escape_string($db, $_POST['identity_no'] ?? '');
    $qualification = mysqli_real_escape_string($db, $_POST['qualification'] ?? '');
    $degree = mysqli_real_escape_string($db, $_POST['degree'] ?? '');
    $experience_year = mysqli_real_escape_string($db, $_POST['experience_year'] ?? '');
    $staff_password = mysqli_real_escape_string($db, $_POST['staff_password'] ?? '');

    // Check if email or phone already exists
    $check_query = "SELECT * FROM user WHERE email = '$email' OR number = '$phone'";
    $result = mysqli_query($db, $check_query);

    if (mysqli_num_rows($result) > 0) {
        echo "<script>alert('Error: Email or Phone Number already exists!'); window.location.href = 'add_staff.php';</script>";
    } else {
        // Handle file upload safely
        $image = "";
        if (!empty($_FILES['image']['name'])) {
            $target_dir = "assets/images/staff_image/";
            $image = basename($_FILES['image']['name']);
            $target_file = $target_dir . $image;

            if (move_uploaded_file($_FILES['image']['tmp_name'], $target_file)) {
                $image = $target_file; // Save file path
            } else {
                echo "<script>alert('Error uploading image.');</script>";
                $image = ""; // Set empty string if upload fails
            }
        }
        ...
        $query = "
        INSERT INTO staff (
            application_date, name, staff_code, father, mother, gender, guardian_occupation,
            religion, religion_category, caste, email, phone, dob, nationality, address, post,
            district, pincode, state, appointment_date, staff_position, identity_type,
            identity_no, qualification, degree, experience_year, staff_password, image
        ) VALUES (
            '$application_date', '$name', '$staff_code', '$father', '$mother', '$gender', 
            '$guardian_occupation', '$religion', '$religion_category', '$caste', '$email', 
            '$phone', '$dob', '$nationality', '$address', '$post', '$district', '$pincode', '$state', 
            '$appointment_date', '$staff_position', '$identity_type', '$identity_no', 
            '$qualification', '$degree', '$experience_year', '$staff_password',
            '$image'
        )";
        ...
        if (mysqli_query($db, $query)) {
            ...
            echo "Error inserting into staff table: " . mysqli_error($db);
        }

        // Insert into 'user' table
        $userquery = "
        INSERT INTO user (
            name, email, number, address, password,role
        ) VALUES (
            '$name', '$email', '$phone', '$address', '$staff_password','$staff_position'
        )";
        ...
        <div class="container">
            <div class="card shadow-sm">
                <div class="text-black text-center border-bottom pb-3">
                    <h4 class="p-3 mt-2"><strong>Fill The Fields For Staff Registration </strong></h4>
                </div>
                <div class="card-body m-2">
                    <!-- <h5 class="card-title text-center mb-4">Staff Information</h5> -->
                    <form method="post">
                        <div class="border-bottom pb-3 mb-3">
                            <h4 class="p-3"><strong>Staff Details</strong></h4>
                            <div class="row">
                                <div class="col-md-8">
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <input type="text" class="form-control" id="staff_name" name="name" placeholder="Enter Staff Name">
                                        </div>
                                    </div>
